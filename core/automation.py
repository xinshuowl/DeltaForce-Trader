...
class GameAutomation:
    ...
    def maximize_quantity(self, clicks: int = 1):
        ...
        clicks=1 (默认): 单次可靠点击, 总耗时 ~0.55s, 比原 2 次 (1.0s+) 显著更快.
        clicks=2: 保守模式, 两次可靠点击. 可通过 config.TIMING["maximize_clicks"] 切换.

        为什么 "单次点击偶尔不成功":
          - 进入上架弹窗后 UI 仍有渐入动画 (~150-220ms), 此时点击会被动画层吞掉.
            旧版 0.08-0.14s 是基于上游 "点击道具后等 0.45s 才判定跳转" 的隐含
            假设 — 上游等待已经吃掉了大部分动画时间, 这里只需补 100ms 兜底.
          - 上游改成密集轮询 (首轮 0.18-0.22s) 之后, has_page_changed 会在弹窗
            "刚开始渐入" 就判定为 True, 此时滑动条按钮命中区域可能还未完全
            生效. 必须把这里的渲染等待加长, 让 maximize_quantity **自己保证**
            点击时弹窗 100% 可交互, 不再依赖上游等待时长.
        ...
        针对性修复 (相比旧的 0.04~0.08s 按下 + 0.06~0.12s 移到点击间隔):
          1. 进入前先 sleep 0.20~0.28s 等 UI 动画渲染完成 (覆盖最长 ~220ms 渐入)
        ...
        # UI 渲染稳定窗口: 弹窗渐入动画实测最长 ~220ms, 在此之前的点击会被
        # 动画层吞掉. 0.20-0.28s 是覆盖 95% 机型的安全下限.
